...
export class OpenAIGarmentDetector implements GarmentDetector {
  constructor(private readonly client: OpenAI, private readonly model = "gpt-5-mini") {}
  ...
  async detect(bytes: Uint8Array, contentType: string): Promise<Detection> {
    ...
    const response = await this.client.responses.create({
      ...
      input: [{
        role: "user",
        content: [
          { type: "input_text", text: "Classify the main garment as exactly one of: top, bottom, dress, outerwear. Return only compact JSON with category and confidence from 0 to 1." },
