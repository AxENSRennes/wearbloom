...
export function createAuth(config: AppConfig, db: Database) {
  ...
  const hasApple = Boolean(appleClientId && config.APPLE_TEAM_ID && config.APPLE_KEY_ID && config.APPLE_PRIVATE_KEY);
        ...
        onLinkAccount: async ({ anonymousUser, newUser }) => {
          ...
          await db.transaction(async (transaction) => {
            await transaction
              ...
              .where(eq(schema.assets.ownerId, oldOwnerId));
            await transaction
              .update(schema.garments)
              .set({ ownerId: newOwnerId })
              .where(eq(schema.garments.ownerId, oldOwnerId));
            await transaction
              .update(schema.referencePhotos)
              .set({ ownerId: newOwnerId })
              .where(eq(schema.referencePhotos.ownerId, oldOwnerId));
            await transaction
              .update(schema.looks)
              .set({ ownerId: newOwnerId })
              .where(eq(schema.looks.ownerId, oldOwnerId));
            await transaction
              .update(schema.renderVariants)
              .set({ ownerId: newOwnerId })
              .where(eq(schema.renderVariants.ownerId, oldOwnerId));
            await transaction
              .update(schema.quotaLedger)
              .set({ ownerId: newOwnerId })
              .where(eq(schema.quotaLedger.ownerId, oldOwnerId));
            await transaction
              .update(schema.idempotencyKeys)
              .set({ ownerId: newOwnerId })
              .where(eq(schema.idempotencyKeys.ownerId, oldOwnerId));
            await transaction
              .update(schema.appAttestChallenges)
              .set({ ownerId: newOwnerId })
              .where(eq(schema.appAttestChallenges.ownerId, oldOwnerId));
            await transaction
              .update(schema.appAttestKeys)
              .set({ ownerId: newOwnerId })
              .where(eq(schema.appAttestKeys.ownerId, oldOwnerId));
            await transaction
            ...
            const [oldEntitlement] = await transaction
              ...
              .limit(1);
            const [newEntitlement] = await transaction
              .select()
              .from(schema.entitlements)
