...
export class AppAttestVerifier {
  ...
  async enroll(input: { ownerId: string; challenge: string; keyId: string; attestation: string }): Promise<void> {
    ...
    await this.db.insert(schema.appAttestKeys).values({
      keyId: input.keyId,
      ...
      set: { ownerId: input.ownerId, publicKey: result.publicKey, environment: result.environment, signCount: 0, lastSeenAt: new Date() },
    });
  }
  ...
  async verifyRequest(input: {
    ...
    this.requireTeamIdentifier();
    const [key] = await this.db.select().from(schema.appAttestKeys).where(and(
      eq(schema.appAttestKeys.keyId, input.keyId),
    ...
    const updated = await this.db.transaction(async (transaction) => {
      const rows = await transaction.update(schema.appAttestKeys).set({
      ...
      if (rows.length) await transaction.delete(schema.appAttestChallenges).where(eq(schema.appAttestChallenges.challenge, input.challenge!));
      return rows.length;
    });
  ...
  private async assertChallenge(ownerId: string, challenge: string): Promise<void> {
    const [row] = await this.db.select().from(schema.appAttestChallenges).where(and(
  ...
  private async consumeChallenge(ownerId: string, challenge: string): Promise<string> {
    ...
    const deleted = await this.db.delete(schema.appAttestChallenges).where(and(
      eq(schema.appAttestChallenges.challenge, challenge),
      eq(schema.appAttestChallenges.ownerId, ownerId),
  ...
  private requireTeamIdentifier(): void {
